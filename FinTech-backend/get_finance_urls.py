...
channel_ids = [
 ...
 # your list of 40 channel IDs

video_urls = []
for ch in channel_ids:
    vids = islice(scrapetube.get_channel(ch), 20)
    for v in vids:
        video_urls.append({
            "channel_id": ch,
...
with open('video_urls.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=["channel_id", "video_url"])
    writer.writeheader()
    writer.writerows(video_urls)

print(f"✅ Saved {len(video_urls)} URLs to video_urls.csv")
